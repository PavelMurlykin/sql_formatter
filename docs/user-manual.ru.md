...
Это ранний прототип. Сейчас доступны программный разбор T-SQL, навигация по токенам, перевод между смещениями и позициями строк, а также модель layout-документа через библиотеку `TSqlFormatter.Core`. Форматирование SQL, рендеринг layout-документа, загрузка конфигурации и команды CLI ещё не реализованы. Готового пакета для установки нет.
...
`SoftLineDoc` означает пробел при размещении в одной строке или перенос при разбиении; `HardLineDoc` — обязательный перенос. `GroupDoc` обозначает содержимое, которое будущий рендерер сможет оставить в одной строке, если оно помещается. `IndentDoc` задаёт неотрицательное число уровней отступа, а `IfBreakDoc(broken, flat)` хранит варианты для разбитого и неразбитого режима. Узлы пока только хранят структуру: API для превращения `Doc` в текст ещё нет.
...
- Модель layout-документа пока не имеет рендерера; создать из неё отформатированный SQL нельзя.
