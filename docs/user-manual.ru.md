...
Это ранний прототип. Через `TSqlFormatter.Core` доступны разбор T-SQL, навигация по токенам, перевод между смещениями и позициями строк, построение и рендеринг layout-документа, а также программное изменение регистра ключевых слов. Структурное форматирование SQL, загрузка конфигурации и команды CLI ещё не реализованы. Готового пакета для установки нет.
...
`ScriptDomSqlFormatter` реализует `ISqlFormatter.Format(source, options, request, cancellationToken)`. Пока он изменяет только регистр токенов, распознанных ScriptDom как ключевые слова; пробелы, строки, комментарии и идентификаторы сохраняются. По умолчанию выбирается `Upper`; доступны `Lower` и `Preserve`:
...
`FormattingOptions` разделяет параметры на `General` (ширина 100, LF, без конечного перевода строки), `Indent` (4 пробела, без табуляции) и `Keywords` (`Upper`). Пока форматтер использует только `Keywords`. `FormatResult` содержит итоговый текст, правки `TextEdit`, диагностики, признаки изменения и успешности разбора.
...
- Форматтер пока не меняет структуру SQL, пробелы и переносы строк.
