...
Пока структурно поддержаны простые списки колонок и одна таблица в `FROM`. При наличии комментариев внутри конструкции либо пока неподдержанного предложения (`WHERE`, `ORDER BY` и т. п.) расположение текста сохраняется, но регистр ключевых слов всё равно может измениться. Завершающая точка с запятой сохраняется.
