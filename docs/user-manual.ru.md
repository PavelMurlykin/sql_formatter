...
Исходные выражения и их порядок сохраняются. Комментарий между элементами `OUTPUT` или неподдержанная форма цели `INTO` оставляют расположение всего оператора без структурной переработки. `OUTPUT` для `MERGE` пока не поддержан.
