...
Пока структурно поддержаны простые списки колонок, одна таблица в `FROM` и базовый `WHERE`. При наличии комментариев внутри конструкции либо пока неподдержанного предложения (`ORDER BY` и т. п.) расположение текста сохраняется, но регистр ключевых слов всё равно может измениться. Завершающая точка с запятой сохраняется.
...
Пока не поддержаны, например, `IS NULL`, `LIKE` и условия в скобках. Для них исходная раскладка сохраняется и может измениться только регистр ключевых слов. Внутренние пробелы в выражениях по сторонам оператора сравнения сохраняются.
...
Поддержаны `INNER JOIN` (и краткое `JOIN`), `LEFT [OUTER] JOIN`, `RIGHT [OUTER] JOIN`, `FULL [OUTER] JOIN`, `CROSS JOIN`, `CROSS APPLY` и `OUTER APPLY`. Каждое соединение цепочки начинается на новой строке. Условие `ON` остаётся в строке соответствующего `JOIN`, его выражение пока сохраняет внутренние пробелы:
