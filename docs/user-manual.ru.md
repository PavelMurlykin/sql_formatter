...
Это ранний прототип. Сейчас доступны программный разбор T-SQL, навигация по токенам и перевод между смещениями и позициями строк через библиотеку `TSqlFormatter.Core`. Форматирование SQL, загрузка конфигурации и команды CLI ещё не реализованы. Готового пакета для установки нет.
