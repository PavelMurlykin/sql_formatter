...
Результат выводится в stdout; вместо отсутствующего аргумента можно передать `-`. `--help` показывает краткую справку. Диагностики идут в stderr. Код завершения `0` означает успешное форматирование, `2` — ошибку разбора SQL, чтения stdin или аргументов; код `1` пока не используется. При ошибке разбора SQL не выводится. CLI использует встроенные значения `Default`, не ищет `.tsqlformatter.json` и пока не принимает имя файла, `--write`, `--check` или флаги настроек.
...
- CLI пока работает только со stdin/stdout; чтение файлов, `--write`, `--check` и поиск конфигурации ещё не реализованы.
