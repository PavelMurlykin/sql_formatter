...
Это ранний прототип. Через `TSqlFormatter.Core` доступны разбор T-SQL, навигация по токенам, классификация комментариев, перевод между смещениями и позициями строк, построение и рендеринг layout-документа, изменение регистра ключевых слов и базовое форматирование `SELECT`, `FROM`, `JOIN` и `APPLY`, включая строчные комментарии после запятых в списке колонок. Загрузка конфигурации и команды CLI ещё не реализованы. Готового пакета для установки нет.
...
Пока структурно поддержаны простые списки колонок, одна таблица в `FROM`, базовый `WHERE`, `GROUP BY`, `HAVING` и `ORDER BY`. Строчные комментарии после запятых в списке колонок и ведущие комментарии перед предложениями поддержаны отдельно; при других комментариях внутри конструкции либо пока неподдержанном предложении исходное расположение сохраняется, но регистр ключевых слов всё равно может измениться. Завершающая точка с запятой сохраняется.
...
Поддержаны несколько таких комментариев в одном списке. Пробел перед `--` нормализуется до одного, переводы строк создаются согласно `GeneralOptions.LineEnding`; сам текст комментария не меняется. Комментарии перед колонкой и блочные комментарии будут обрабатываться на следующих этапах.

### Ведущие комментарии перед предложениями

Один или несколько соседних строчных комментариев непосредственно перед `FROM`, `WHERE`, `GROUP BY`, `HAVING` или `ORDER BY` сохраняются на отдельных строках перед соответствующим предложением:
