...
Результат выводится в stdout; вместо отсутствующего аргумента можно передать `-`. `--help` показывает краткую справку. Диагностики идут в stderr. Код завершения `0` означает успешное форматирование, `2` — ошибку разбора SQL, ввода-вывода или аргументов. Код `1` используется только в режиме `--check`, когда форматирование требуется. При ошибке разбора SQL не выводится. CLI использует встроенные значения `Default`, не ищет `.tsqlformatter.json` и пока не принимает флаги настроек.
...
Код `0` означает, что файл уже отформатирован; `1` — что форматирование нужно; `2` — ошибку SQL, чтения или аргументов. `--check` не пишет в файл и оставляет stdout пустым. Для stdin (`-`) он пока недоступен.
...
- CLI принимает stdin или один файл; `--write` и `--check` доступны только для файла. Поиск конфигурации ещё не реализован.
